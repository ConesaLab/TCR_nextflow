...
summary_table_TCR <- do.call(rbind, lapply(names(clonotypes_tcr), function(i){
  ...
  CDR3aa_minlength = min(nchar(clonotypes_tcr[[i]]$aaSeqCDR3))
  ...
  cbind(sample,TCRclones,filterby_readcount,CDR3aa_minlength,filterby_CDR3aa,filterby_both)
  }))
...
clones_filtered <- clones_TCR_filtered
save(clones_filtered, file = file.path(params$workDir,
                                           "clones_TCR_filtered.Rda"))
```
...
colnames(TCR_SE) <- "Shannon-Evenness"
```
...
lapply(names(clones_TCR_filtered), function(x){
  write.table(clones_TCR_filtered[[x]],
              file = file.path(params$workDir, "clones_TCR_filtered", 
                               paste0(x, ".txt")),
