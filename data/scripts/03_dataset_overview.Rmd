...
TCR_reads <- alignment_reports[,c("sampleID",
...
SampleInfo <- merge(SampleInfo, TCR_reads[,c("sampleID", "TCRreads")],
...
```{r, results='hide'}
# TCR plot
save(p1_readsTCR, file = file.path(params$workDir, "03_boxplot3_reads_TCR.Rda"))
png(file.path(params$outputDir, "03_boxplot3_reads_TCR.png"), height = 30, width = 30, units = "cm", res = 300)
p1_readsTCR
dev.off()
...
TCR_clones <- assemble_reports[,c("sampleID", "TRA.chains", "TRB.chains", "TRD.chains", "TRG.chains")]
TCR_clones$Clones <- rowSums(TCR_clones[,2:ncol(TCR_clones)])

SampleInfo <- merge(SampleInfo, TCR_clones[,c("sampleID", "Clones")],
                    by.x = "SampleID", by.y = "sampleID", sort = F)
...
```{r, results='hide'}
# TCR plot
save(p1_clones_filtered_TCR, file = file.path(params$workDir, "03_boxplot4_clones_TCR.Rda"))
png(file.path(params$outputDir, "03_boxplot4_clones_TCR.png"), height = 30, width = 30, units = "cm", res = 300)
p1_clones_filtered_TCR
dev.off()
...
### TCR Shannon-Evenness
```{r}
summary_table_TCR <- read.delim("02_filtering-evenness_summarytable_TCR.csv", check.names = FALSE)
summary_table_TCR <- summary_table_TCR[,c("sample", "Shannon-Evenness")]
summary_table_TCR$`Shannon-Evenness` <- round(summary_table_TCR$`Shannon-Evenness`, 4)
colnames(summary_table_TCR) <- c("sample", "TCRshannonevenness")
...
```{r, results='hide'}
# TCR plot
save(p1_shannon_filtered_TCR, file = file.path(params$workDir, "03_boxplot5_shannonevenness_TCR.Rda"))
png(file.path(params$outputDir, "03_boxplot5_shannonevenness_TCR.png"), height = 30, width = 30, units = "cm", res = 300)
p1_shannon_filtered_TCR
dev.off()
...
* TCRshannonevenness: Shannon eveness of TCR clones (TRA+TRB+TRD+TRG)
